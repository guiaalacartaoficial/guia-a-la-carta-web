...
const Navbar = () => {
  ...
  // Detectar scroll para transiciÃ³n transparente â†’ sÃ³lido
  useEffect(() => {
    ...
    <header className={`navbar-header ${scrolled ? 'scrolled' : ''} ${isHome && !scrolled ? 'transparent' : ''}`}>
      <div className="container navbar-container">
        <Link to="/" className="navbar-logo" onClick={closeMenu}>
          <img src="/logo.png" alt="GuÃ­a a la Carta" />
          <span>GuÃ­a a la Carta</span>
        </Link>

        <button className="mobile-toggle" onClick={() => setIsOpen(!isOpen)} aria-label="MenÃº">
        ...
        <nav className={`navbar-nav ${isOpen ? 'active' : ''}`}>
          ...
          <NavLink to="/guias" className={({ isActive }) => `nav-link ${isActive ? 'active' : ''}`} onClick={closeMenu}>Nuestros GuÃ­as</NavLink>
          ...
          <div className="nav-dropdown">
            ...
            <div className={`nav-dropdown-menu ${postOpen ? 'show' : ''}`}>
              <NavLink to="/postular-guia" onClick={closeMenu} className="dropdown-item">Postular como GuÃ­a</NavLink>
              <NavLink to="/postular-estudiante" onClick={closeMenu} className="dropdown-item">Postular como Estudiante</NavLink>
            </div>
          </div>

          {/* Dropdown GestiÃ³n de Servicios */}
          <div className="nav-dropdown">
            <button className="nav-link nav-dropdown-toggle" onClick={() => { setSociosOpen(!sociosOpen); setPostOpen(false); }}>
              GestiÃ³n de Servicios <ChevronDown size={16} className={`chevron ${sociosOpen ? 'open' : ''}`} />
            </button>
            <div className={`nav-dropdown-menu ${sociosOpen ? 'show' : ''}`}>
              <NavLink to="/disponibilidad" onClick={closeMenu} className="dropdown-item">Mi Disponibilidad (GuÃ­a)</NavLink>
            </div>
          </div>
          ...
          <NavLink to="/reservar" className={({ isActive }) => `nav-link nav-highlight ${isActive ? 'active' : ''}`} onClick={closeMenu}>Reservar GuÃ­a</NavLink>
